  ...
  <div class="addQuest">
    ...
    >
      <div slot="title" class="title">
        ...
        <span class="delBtn" @click="handleDelete(cardIndex)">x</span>
      </div>
      <!-- 各项选择 -->
      <div class="body">
        <div class="setting">
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      ...
      let res = window.localStorage.setItem(
        "questList",
        JSON.stringify(this.questList)
      );
      this.$Message.success("本地保存成功");
    },
    handleDelete(cardIndex) {
      this.$Message.warning("删除 " + this.questList[cardIndex].title);
  ...
  created() {},
  mounted() {
    let questList = window.localStorage.getItem("questList");
    if (questList) {
      this.questList = JSON.parse(questList);
      console.log("mounted questList", this.questList);
      this.$Message.success("本地获取成功");
    }
  }
};
</script>
